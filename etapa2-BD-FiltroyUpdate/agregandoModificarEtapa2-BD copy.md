...
* **Validar que el DNI/CUIT no se repita al modificar:** antes de `actualizarPersona()`, hacer `SELECT id FROM personas WHERE dni = ? AND id != ?` y si existe, redirigir con un mensaje de error.
* **Resaltar la fila modificada:** después del `header("Location: resultado.php?modificado=5")`, en `resultado.php` leer `$_GET['modificado']` y agregar `class="table-success"` a esa fila.
* **Historial de cambios:** crear una tabla `personas_historial` y en `actualizarPersona()` primero copiar los datos viejos allí (auditoría).
* **Modificar desde el filtro:** el link `index.php?id=X` funciona igual aunque vengas de `resultado.php?busqueda=Jul`. Al volver, ¿se mantiene el filtro? Probá agregando `?busqueda=...` al `header("Location: resultado.php")` si querés conservarlo.
* **Comparar con la Etapa 3:** mirá `etapa3-BD-tablas/formCiudad.php` — allí el mismo patrón (`?id=` → `obtenerCiudadPorId` → `actualizarCiudad`) se aplica a ciudades. Es el mismo código, solo cambian los nombres de los campos.
