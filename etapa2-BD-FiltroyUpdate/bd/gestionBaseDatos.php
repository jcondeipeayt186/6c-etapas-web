...
function actualizarPersona($conexion, $id, $datos) {
    $sql = "UPDATE personas
            SET nombre = ?, apellido = ?, dni = ?, cuit = ?, fecha_nacimiento = ?, email = ?, telefono = ?, direccion = ?, ciudad = ?, provincia = ?, codigo_postal = ?, observaciones = ?
            WHERE id = ?";
    ...
    // El ultimo ? es el id del WHERE, por eso va al final del array
    return $resultado;
}
